...
def Gauss(A, B):
    n, m = np.shape(A)
    solution = np.column_stack((A, B))

    Xgauss = ResolutionSystTriSup(ReductionGauss(solution))
...
for i in taille:
    ...
    # A = np.array([[0.0,1.0,1.0,1.0],[0.0,0.0,-3.0,2.0],[-1.0,-1.0,0.0,-3.0],[1.0,-1.0,4.0,9.0]])
    # B = np.array([[1.0],[1.0],[2.0],[-8.0]])
    n = len(A)
    list_nb.append(n)
...
def graph_tps():
    ...
    plt.ylabel("Temps d'éxécution du calcul (s)")
...
def graph_erreur():
    ...
    plt.title("Erreur des différentes fonction en fonction de la taille de la matrice")
